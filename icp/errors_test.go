...
func Test_NewMultiError_1(t *testing.T) {
	...
	assert.Equal(t, tmp[0], "github.com/gjvnq/libICP/icp.Test_NewMultiError_1 (errors_test.go:?): ERR_OK: hi")
...
func Test_NewMultiError_2(t *testing.T) {
	...
	r := regexp.MustCompile("[.]go:[0-9]*")
	tmp[0] = r.ReplaceAllString(tmp[0], ".go:?")
	assert.Equal(t, tmp[0], "github.com/gjvnq/libICP/icp.Test_NewMultiError_2 (errors_test.go:?): ERR_OK: hi")
	assert.EqualValues(t, ERR_OK, merr.Code())
}
