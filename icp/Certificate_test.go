...
func Test_CheckAgainstIssuerCRL_1(t *testing.T) {
	...
	// Try to parse
	err := ca.ProcessCRL(crls[0])
	require.Nil(t, err)
	assert.Nil(t, ca.CRLLastError())

	// Check
	fulano.CheckAgainstIssuerCRL(&ca)
	assert.EqualValues(t, CRL_NOT_REVOKED, fulano.CRLStatus())
	assert.Equal(t, ca.CRLThisUpdate(), fulano.CRLLastCheck())
}
...
func Test_CheckAgainstIssuerCRL_2(t *testing.T) {
	...
	// Check
	fulano.CheckAgainstIssuerCRL(&ca)
	assert.EqualValues(t, CRL_REVOKED, fulano.CRLStatus())
	assert.Equal(t, ca.CRLThisUpdate(), fulano.CRLLastCheck())
}
