...
func Test_NewCertificateFromFile_1(t *testing.T) {
	...
	assert.True(t, cert.KeyUsage().Exists)
	assert.False(t, cert.KeyUsage().DigitalSignature)
	assert.False(t, cert.KeyUsage().NonRepudiation)
	assert.False(t, cert.KeyUsage().KeyEncipherment)
	assert.False(t, cert.KeyUsage().DataEncipherment)
	assert.False(t, cert.KeyUsage().KeyAgreement)
	assert.True(t, cert.KeyUsage().KeyCertSign)
	assert.True(t, cert.KeyUsage().CRLSign)
	assert.True(t, cert.BasicConstraints().Exists)

	// Finish
	os.Remove(tmp_file.Name())
}

func Test_check_against_issuer_crl_1(t *testing.T) {
	// Get cert
	...
	status, age := fulano.CRLStatus()
	assert.EqualValues(t, CRL_NOT_REVOKED, status)
	assert.Equal(t, ca.crl_this_update(), age)
}

func Test_check_against_issuer_crl_2(t *testing.T) {
	// Get cert
	certs, errs := NewCertificateFromFile("data/test-chain/intermediate/fakebank/certs/fakebank-ca.crt.pem")
	require.Nil(t, errs)
	require.Equal(t, 1, len(certs))
	...
	fulano.check_against_issuer_crl(&ca)
	status, age := fulano.CRLStatus()
	assert.EqualValues(t, CRL_REVOKED, status)
	assert.Equal(t, ca.crl_this_update(), age)
}
