...
func Test_parse_crl(t *testing.T) {
	// Get cert
	certs, errs := NewCertificateFromBytes([]byte(ROOT_CA_BR_ICP_V2))
	require.Nil(t, errs)

	// Get CRL
	crl, err := base64.StdEncoding.DecodeString(crl_raiz_v2)
	require.Nil(t, err)

	// Try to parse
	err = certs[0].parse_crl(crl)
	assert.Nil(t, err)
	assert.Nil(t, certs[0].CRLLastError())

	store := NewCAStore(true)
	store.WaitDownloads()
	println("GOOOOOOOOT IT")
}
